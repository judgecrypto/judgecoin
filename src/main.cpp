...
const string strMessageMagic = "Judgecoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // judgecoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // judgecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// judgecoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes judgecoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // judgecoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // judgecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // judgecoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // judgecoin: track money supply and mint amount info
...
// judgecoin: total coin age spent in transaction, in the unit of coin-days.
...
// judgecoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // judgecoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // judgecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // judgecoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // judgecoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        {                  
        // Coinbase pos fix output should be empty if proof-of-stake block
        if (pindexBest->nHeight >= 6000 && vtx[0].vout.size() != 1 || !vtx[0].vout[0].IsEmpty())
        ...
        // Judgecoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // judgecoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // judgecoin: check proof-of-stake
    ...
    // judgecoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // judgecoin: getblocks may not obtain the ancestor block rejected
    ...
    // judgecoin: if responsible for sync-checkpoint send it
...
// judgecoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "judgecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "You be the judge, I be at your command";
        CTransaction txNew;
        txNew.nTime = 1403793711;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403793711;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 7262758;
        if(fTestNet)
        {
            block.nNonce   = 2013163;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x5d1023c5dc4ebf59c228dda3e8a4b203d89606cc3366983703327ada9d9c429c"));
        block.print();
        ...
        // judgecoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // judgecoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // judgecoin: tell downloading node about the latest block if it's
